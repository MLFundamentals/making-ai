...
- [ ] `V-4` `Level 4-2` **성공률의 아래 꼬리** — 러너에 9.8% 기록이 있는데,
      2026-08-22 에 컨테이너에서 68회를 돌려도 **47~64% 로 재현되지 않았다.**
      한 회에 8.6초가 들어(에피소드 10,000회 · `is_slippery=True`) 표본을 더 못 쌓았다.
      → **Colab 에서 이 노트북만 대여섯 번 반복 실행하고 성공률을 적어 둘 것.**
        20% 아래가 한 번이라도 나오면 「드문 꼬리」이고, 안 나오면
        9.8% 는 러너 환경에서만 나오는 값이다. 어느 쪽이든 **하한은 걸지 않는다**
      → 원고는 「대략 60~70%」라고 쓴다. 컨테이너 실측 최저가 이미 47% 다
