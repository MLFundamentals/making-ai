...
CASES = [
    ...
    dict(chapter=7, section="VII-1", file="Generative AI_Text Generator.ipynb",
         ...
         note="버튼 대역의 첫 실전 시험. 이 노트북은 끝이 button.on_click(...) + display(...) "
              ...
              "⚠ 아직 button_max_chars 를 걸지 않았다. 실측 두 번은 166자·165자다"
              "(2026-08-20). 값이 안정돼 보이지만 **표본이 둘뿐이라 아직 선을 긋지 않는다** — "
              "다음 달 예약 실행이 세 번째 값을 준다. "
              "⚠ 이 166자는 **생성된 문장의 길이가 아니다.** 노트북이 마침표마다 '<br>' 를 "
              "붙이고(.replace) 머리말과 프롬프트가 더해진 값이다. 상한을 정할 때 이 부풀림을 "
              "감안할 것. 그리고 VI-11 과 위험의 성격이 다르다 — VI-11 은 max_length 라는 "
